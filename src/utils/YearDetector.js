...
// import { promisify } from 'util'; // Currently unused but kept for future use
...
export class YearDetector {
  ...
  static async detectYear(epubPath) {
    ...
    const tempDir = path.join(
      os.tmpdir(),
      `epub-detect-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`
    );
  ...
  static async extractMinimalFiles(epubPath, targetDir) {
    // const _openZip = promisify(yauzl.open); // Currently unused but kept for future use
        ...
        zipfile.on('entry', (entry) => {
          ...
          if (shouldExtract && !entry.fileName.endsWith('/')) {
            ...
            zipfile.openReadStream(entry, (streamErr, readStream) => {
              if (streamErr) {
                logger.warn(
                  `Failed to open read stream for ${entry.fileName}: ${streamErr.message}`
                );
                zipfile.readEntry();
                return;
              }
              ...
              fs.mkdir(fileDir, { recursive: true })
                ...
                .catch((mkdirError) => {
                  logger.warn(
                    `Failed to create directory for ${entry.fileName}: ${mkdirError.message}`
                  );
                  zipfile.readEntry();
                });
            });
  ...
  static async detectFromTitlePage(tempDir) {
    ...
    try {
      ...
      const titleFiles = files.filter((f) => f.includes('212') && f.endsWith('.xhtml'));

      // Procesar cada archivo de título potencial
      for (const titleFile of titleFiles) {
        try {
          const content = await fs.readFile(path.join(oebpsDir, titleFile), 'utf-8');
